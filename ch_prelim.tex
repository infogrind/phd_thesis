...
discrete-time memoryless sources across a discrete-time memoryless channels,
where $n$~symbols can be transmitted across the channel for each $k$~source
symbols.  The goal of the present chapter is to establish the playing field by
providing all necessary definitions and by deriving the known theoretical
limits.
...
limited on the additive white Gaussian noise channel, its goal is to find
...
Of particular interest is the standard Gaussian channel. 
\begin{definition}
  \label{def:awgn}
...
In general, the same code may not be the optimal one for different values
of~\snr. Hence we use the concept of coding schemes.
\begin{definition}
...
\begin{theorem}
  ...
  the expected distortion~$D$ and expected cost~$P$ are related by
  \begin{equation}
...
\end{definition}
As \secref{optmindelfb} will show, if the channel has noiseless feedback then a
...
The following lemma provides a necessary condition for the asymptotic optimality
of a communication system.

\begin{lemma}
  \label{lem:asymptoptinputs}
  Consider a sequence of zero-mean input distributions $\{p(x^n)\}$ on the
  Gaussian channel with covariance matrix sequence~$\{K\}$ satisfying $\Tr K /
  (n \szq) = \snr$ and 
  \begin{equation*}
