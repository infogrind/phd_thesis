...
\begin{example}[Exponential]\label{ex:expchannel}
  ...
  $a+b$, as has been shown in \cite{Verdu1996}. 
...
\begin{example}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[every node/.style={font=\scriptsize}]
        ...
        \draw (x0) -- node[below] {$1-\e$} (y0);
        \draw (x2) -- node[above] {$1-\e$} (y2);
        \draw (x0) -- node[below,near start] {$\e$} (y2);
        \draw (x2) -- node[above,near start] {$\e$} (y0);
        \draw (x1) -- node[above,sloped,very near start] {$0.5$} (y2);
...
\subfloat[$\Phi_0 > 0$]{
      ...
      \caption{The three possible cases for $\Phi_0$ of \remref{verduphi}.}
  \label{fig:verduphi}
\end{figure}
