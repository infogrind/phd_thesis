...
%        \draw[gridstyle] (-0.01,-0.01) grid[xstep=0.16,ystep=0.1] (0.81,0.51);
%
%        % Axes
%        \draw[->] (-0.02,0) -- (0.82,0) node[right] {$C(P)$};
%        \draw[->] (0,-0.02) -- (0, 0.52) node[above] {$P$};
%
%        % The function to plot
%        \draw[color=black,domain=0.23:0.61] plot[id=bsccap1] 
%        function{0.6*(-0.47 + (-1.64+1.51*x) * log(1.14-1.05*x)
%        + (0.2-1.51*x) * log(-0.14 + 1.05*x))};
%        \draw[color=black,domain=0.61:0.77] plot[id=bsccap2]
%        function{0.6*0.53};
%        \draw[color=red,domain=0:0.71] plot[id=tangent1]
%        function{0.6*x};
%      \end{tikzpicture}
%  \end{center}
