...
(in the sense of \defref{optimality}) the cost measure and the distortion
...
\lemref{gastparcost}. This criterion is applicable whether
...
Refining \lemref{gastparrd}, we give a condition for a test
...
\begin{proof}
  ...
  \[ \Ip(X;Y) = C(P^*). \]
  By \lemref{gastparcost}, a necessary condition for this is
...
\begin{example}
  ...
  in \figref{edgecapacity}, has a sharp edge at $P=2$. It is thus clear that
...
This result shows that the extra requirement that the capacity per unit cost
be achieved restricts the cost measures allowed by \lemref{gastparcost} to
those with $\beta = 0$.
...
This result shows that the extra requirement that the maximum fidelity per rate
be achieved restricts the fidelity measures allowed by \lemref{gastparrd} to
...
\newcommand{\myaxes}{
        \draw[->] (-0.1,0) -- (2.1,0) node[right] {$R(\Phi)$};
        \draw[->] (0,-2.8) -- (0, 2.8) node[above] {$\Phi$};
}
