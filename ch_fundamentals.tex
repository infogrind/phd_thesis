...
\end{itemize}
The parameters $\ts$, $\tc$, $n$, and $k$ are given as part of the problem
setting. To match the number of channel inputs produced by the encoder to the
rate at which the channel accepts them, they must satisfy $n/k = \ts / \tc$. 
...
\begin{proof}
  ...
  cost at most~$P$ and can transmit reliably $C(P) - \e'$ bits per channel use
  ...
  This follows directly from the assumption if $\e'$ is chosen small enough.
\end{proof}

\thmref{separationforward} states that under the conditions of the source coding
...
nC(P)$ and thus lead to an optimal communication system in the sense of
\defref{optimality}. In the limit, the sequence of separation codes thus
fulfills all the conditions of \thmref{optimalityconditions}.
...
Verd\'u~\cite{ShamaiV1997}. Similar arguments to those in the paper probably
...
achieving distribution (cf.~\cite{ShamaiV1997}); this is a question that
certainly merits further investigation.


\subsection{Alternatives to Separation-Based Codes}

The separation theorem uses separate source and channel codes to prove the
...
system. To be precise, we define first what we exactly mean by delay.
...
channel codes must be sacrificed in favor of codes that are designed jointly for
a particular source and a channel. As we will see shortly, there exist of such
joint source-channel codes that perform as good as any separation-based code,
yet cause only minimal delay.
...
distortion region for such codes. There have certainly been attempts to refine
the bound of \thmref{separationconverse} for delay limited codes, most notably
Ziv and Zakai's observation that tighter bounds can be obtained by replacing the
logarithm in the mutual information by a different function, as long as this
function satisfies certain constraints~\cite{ZivZ1973}. This can result in outer
bounds that take into account the delay. However, no bounds have been found
(yet) using this method that are provably tight for a given delay constraint,
not even for the canonical case of a Gaussian source and channel, which is the
