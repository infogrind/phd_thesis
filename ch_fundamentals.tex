...
information theory, Shannon's source coding and channel coding theorems, do not
apply if one is restricted to operate on sequences of short length. In fact, the
whole notion of typicality, a standard ingredient in information theoretic
achievability proofs, does not apply if one cannot assume arbitrarily long block
lengths. 
...
longer guaranteed. \secref{delayconstraint} thus looks at the consequences of
limited delay and shows that this topic still has a lot of open questions.
\secref{gaussian} focuses on the particular case of a Gaussian source and
...
\secref{gaussuncoded}. If the channel allows several transmissions for each
...
scheme to come in \chapref{mindelbwex}. 
