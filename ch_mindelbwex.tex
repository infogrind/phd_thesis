\chapter[A Hybrid Communication Strategy for Gaussian Channels]{A Hybrid
Communication Strategy for Gaussian Channels\looseness=-1}\label{ch:mindelbwex}
...
\begin{proof}
  ...
  $\exp\{-c \snr^\e\}$ is strictly decreasing in~$\snr^\e$, there is at
  most one solution to~\eqref{eq:epsequation} in~$\snr^\e$.  Assume now
  $\snr^\e$ is as in~\eqref{eq:epssolution}. Then
  \begin{equation*}
...
\end{equation}
where $\phi(r)$ is the pdf of a real Gaussian random variable with variance
...
(see~\cite[\Spg 26.2.12]{AbramowitzS1964}). Letting
