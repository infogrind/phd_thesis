...
\section{Two Alternative Perspectives}

The problem how to transmit a single symbol from a continuous-valued source
across an AWGN channel in $n$~channel uses can be approached from two different
perspectives. The first perspective is to take inspiration from the feedback
case of \exref{gaussfb}. The second, more traditional perspective is a geometric
analysis of the signal constellation. This chapter starts with a presentation of
both points of view and shows how they both lead tho the same tradeoff. Based on
this, a concrete transmission scheme is presented and analyzed in the
subsequent sections. 


\subsection{The Point of View from Feedback}

The lesson from \exref{gaussfb} is that uncoded transmission is optimal if the
transmitter knows the receiver's estimate given the previous channel outputs.
Because of the inherent randomness of uncoded transmission, however, the
transmitter has little chance of guessing the receiver's estimate correctly
after a round of uncoded transmission if there is no feedback. Nevertheless, if
the transmission in the first $n-1$ channel uses was such that the transmitter
has a reasonably good idea of the receiver's estimate (by using some form of
coding, for example) then the transmitter can send the estimation error uncoded
in the last channel use, even if there is no feedback. 
...
\Eh = E - \Eh$. Now write the overall SDR as
\begin{equation*}
  \sdr = \frac{\ssq}{\mse} = \frac{\ssq}{\E[(\Sh' - S)^2]} \cdot
  \frac{\E[(\Sh' - S)^2]}{\msee}.
\end{equation*}
The first term on the right hand side is the SDR after $n-1$ channel uses. The
second term, equal to $\E[E^2]/\msee$, is the SDR resulting from the
...
Assume then that a perfect source code combined with a perfect channel code is
...
the associated distortion $\E[E^2]$ scales as $\snr^{-(n-1)}$. If $E$ is
transmitted uncoded in the $n\th$~channel use and the receiver lets $\Sh = Q +
\Eh$, the overall SDR is
\begin{equation*}
...
\begin{equation}
  \label{eq:sdrqesep}
  \sdr = \frac{\ssq}{\E[E^2]} \cdot \frac{\E[E^2]}{\mseq + \E[(\Eh -
  E)^2]}.
\end{equation}
...
$\mseq$ decreases too slowly, the eq:approximation~\eqref{eq:sdrscalingapprox}
...
The conclusion from all this is that error free communication in the first
$n-1$~channel uses is not necessary to take advantage of uncoded communication
in the last channel use. All that is needed is that the error from the first
$n-1$~channel uses is dominated by that of the last channel use as $\snr \goesto
\infty$. Furthermore, the choice of how $S$ is split up into parts transmitted
in the first $n-1$ and in the $n\th$ channel use, respectively, represents a
tradeoff, and SDR scaling is optimized when the two errors scale the same. The
transmission schemes developed in this chapter are designed with this principle
in mind.
...
3$ in Figures~\ref{fig:hybridlocus2} and~\ref{fig:hybridlocus3}, respectively.
...
\begin{figure}
  \centerline{%
  \subfloat[$\beta = 2$]{\label{fig:hybridlocus2}%
  \input{figures/hybridlocus2.tex_t}}
  \hfil
  \subfloat[$\beta = 3$]{\label{fig:hybridlocus3}%
