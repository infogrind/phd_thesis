...
$\e = \e(\snr)$ is a positive function of~$\snr$. This results in no loss of
generality, since for an arbitrary positive function~$f$ we can set $\e(\snr) =
1-2 \log(f(\snr))/\log\snr$ to get $\beta(\snr) = \lceil f(\snr) \rceil$.
Writing $\beta$ in this form will slightly simplify the mathematical derivations
to follow. Note that we can bound $\beta$ by $\snr^{(1-\e)/2} \le \beta \le
\snr^{(1-\e)/2} + 1$. For any $n > 0$ there exist thus a constant~$c$ such that
for large values of~\snr
\begin{equation}
  \label{eq:betabound}
  \snr^{n(1-\e)/2} \le \beta^n \le c \snr^{n(1-\e)/2},
\end{equation}
a property that will prove useful in the derivations to come.
...
\begin{proof}[Proof of Theorem~\ref{thm:scalinglb}]
  For notational symplicity define
  \begin{align*}
    l_1(\snr, \e) &= \snr^{-n+(n-1)\e} \quad\text{and} \\
    l_2(\snr,\e) &= \snr^{-1+\e/2} \exp\{-\snr^\e/k\}.
  \end{align*}
...
\begin{notebox}
  Write introductory paragraphs here.
\end{notebox}
...
\begin{notebox}
  Any source with bounded support can just be scaled down to lie in this
  interval. This just changes the \sdr\ by a constant and doesn't influence the
  scaling.
\end{notebox}
