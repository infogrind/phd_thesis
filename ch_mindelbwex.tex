...
error for a given SNR (given by \thmref{separationconverse}) may not be
...
the source and noise variance, respectively, and $D$ and $P$ are the average squared error and average transmit power, respectively. In the previous chapter we have seen that when k source symbols are encoded into n channel input symbols, the SDR and the SNR are related by
\begin{equation}
...
defined formally in \appref{asymptotic}. Informally, we can say that $O(f(x))$
...
\todo: For analog sources, the same asymptotic limit applies.
...
The question whether this is a fundamental limit is unfortunately not answered
by this thesis.  Nevertheless, \secref{gensdrub} prepares the way towards
...
\figvref{tangentproj}). Writing $\sh = s_0 + \Delta$, the estimation error is
...
constellation resulting from the communication scheme of \secref{scalarquant},
