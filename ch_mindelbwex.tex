\chapter{Minimal-Delay Codes for Bandwidth Expansion}\label{ch:mindelbwex}
...
squared error for a given SNR (given by \thmref{separationconverse}) is not
achievable (see \chapref{fundamentals}), we look in this chapter at the simpler
...
is defined formally in \appref{asymptotic}. Informally, we can say that
...
The communication scheme studied in this chapter is easy to describe. It
...
not entirely new (see \secref{mindelbwexhist} for references to previous work),
this chapter provides the first exact analysis of the resulting SDR scaling and
the optimization thereof.
...
inspiration from the feedback case of \exref{gaussfb} in \chapref{fundamentals}.
The second, more traditional perspective is a geometric analysis of the signal
constellation.  \secref{twoperspectives} starts this chapter by presenting the
two points of view and showing how they both lead to the same tradeoff.
\secref{scalarquant} then formally introduces the hybrid communication scheme
that is at the heart of this chapter. It is shown that this scheme cannot
achieve an SDR that scales better than $\snr^n / (\log\snr)^{n-1}$, regardless
of the decoder used, and a simple suboptimal decoder in fact attains this
scaling.  In \secref{latticequant}, the communication scheme of
\secref{scalarquant} is extended to encode \emph{blocks} of $m$~source symbols
into $mn$~channel inputs using lattice quantizers. It turns out that even when
powerful high dimensional lattices are used, the SDR for any finite block
length~$m$ scales just like when $m = 1$ (although the multiplicative constants
may be larger). \secref{genbwexp} extends the scheme of \secref{scalarquant} in
a different way, namely to encode $k$~source symbols into $n$~channel inputs,
where $k \le n$. For this case, the achievable SDR is shown to scale as
$\snr^{n/k} /(\log\snr)^{(n-k)/k}$. 
...
The question whether this is a fundamental limit is unfortunately not resolved
in this thesis.  Nevertheless, \secref{gensdrub} prepares the way towards
...
briefly mentioned at the end of \chapref{fundamentals}. 
...
The argument just given assumed that the transmitter knew the receiver's
...
because $Q$~is obtained by quantizing the source with a too fine resolution),
...
the uncoded transmission is lost.  If, on the other hand, $\mseq$ decreases to
...
striking similarity between Equations~\ref{eq:sdrconstantstretch}
and~\ref{eq:sdrscalingapprox2}: both equations express the SDR as the product of
...
use.\footnote{The main results of \secref{scalarquant} have been published
...
\begin{figure}
  ...
  $n = 4$. The triangles represent the scaling operations of
  Equations~\ref{eq:QEdefB} and~\ref{eq:Xdef}.}
...
\begin{remark}
  ...
  illustrated on \figref{hybridlocus}. Each of the segments of length
...
mean squared error of the hybrid transmission strategy considered, each of which
is tighter for a different class of~$\e$. They hold regardless of the decoder
used.  (The $\Omega$-notation is defined in Appendix~\ref{app:asymptotic}.)
...
\begin{remark}
  \label{rem:geomlb}
  \figref{sdrubproof} nicely illustrates the connection to the geometric
  argument given in \secref{twoperspectives}. The MSE is lower bounded by
...
\begin{lemma}
  ...
  arbitrary real constants $a$, $b>0$, and $c > 0$, it holds that
  \begin{equation}
...
the simulation results on \figref{scalarcomparison}.  The remainder of this
...
\begin{figure}
  ...
  for smaller~$\e$ the opposite holds. The theoretically optimal curve
...
Ziv's lower bound on the mean squared error introduced in \secref{scalarquant}
...
\snr^{(1-\e)/2} \rceil$ without loss of generality (cf.~\remref{betaepswlog}).)
...
scalar scheme of \secref{scalarquant}. Hence \thmref{scalarachievability}
...
Like in \secref{scalarquant}, a combination of ML decoder (for the lattice
...
$(\log\snr)^{1/k}$ for each of the $n-k$ channel inputs that carry quantized
...
forward in \secref{twoperspectives} make the use of discrete channel inputs
...
constellation resulting from the communication scheme of \secref{scalarquant} to
...
\begin{figure}
  ...
  presented in \secref{scalarquant} is compared to Shannon's original
...
Much of the more recent work on minimal-delay joint source-channel codes is due
...
For $n=2$, the scheme of \secref{scalarquant} is almost identical to the HSQLC
...
\end{proof}
  ...
  \begin{example}
    ...
    This is exactly the quantizer used in \secref{scalarquant}.
  \end{example}
