...
the same type achieves a better SDR scaling. In \secref{latticequant}, the
communication scheme of \secref{scalarquant} is extended to encode \emph{blocks}
...
more fundamental limit? \secref{gensdrub} prepares the way towards answering
...
length $1/\beta^{n-1}$, as is illustrated in \figref{sourcepartition}. The $Q_i$
...
The following results make these reflections precise. They indicate how to
choose $\beta$ as a function of the SNR to optimize the MSE scaling.
...
\begin{figure}
  ...
  \lemref{lowerbound1}. The length of each $\I_j^\Delta$ is $\beta^{-(n-1)} -
...
\begin{remark}
  ...
  argument given in \secref{mindelintro}. The MSE is lower bounded by
...
\begin{equation}
  \label{eq:scalarachievablescaling}
...
scalar scheme of \secref{scalarquant}. Hence \eqref{eq:scalarachievablescaling}
applies to lattice quantizers as well. 

\figref{sim_latticeperf} shows SDR curves achieved using lattices of various
...
$(\gamma^2/P) \szq = \gamma^2/\snr$. To get explicit bounds that
