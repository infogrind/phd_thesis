...
source-channel coding schemes. Its main design goal was to allow rapid testing
...
polymorphism. Moreover, this structure could easily be translated to
...
Thereafter, \secref{reference} provides a detailed reference manual. 
...
|OutputModule| (or from one of its derived classes). Any class derived from
|OutputModule| must implement the following two methods.
...
|TheoreticalScheme|, it just calls |update_mse()|, which classes derived from
it (such as |ShannonScheme|) must implement. 
