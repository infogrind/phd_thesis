...
provides a detailed reference and usage manual.  Lastly, \secref{implementation}
contains a few more remarks on the implementation.

Before introducing \jscsim, though, the chapter starts with a short example that
illustrates why object oriented programming is almost indispensable for writing
good simulations.
...
want to see how to \emph{use} JSCsim and care less about what goes on behind the
scenes. The second subsection, starting on page~\pageref{sec:impoverview},
presents \jscsim\ from the developer's point of view by presenting the
architecture in a top-down manner. Depending on the reader's interests, she may
safely skip one or the other subsection. 
...
that analyzes its performance, but shouldn't have to think about both at the
same time.
For this reason, the core classes that make up \jscsim\ are divided into two
categories. In the first category are the \emph{scheme classes} (or simply
called 'schemes'), which are classes that implement communication systems and
have the class |Scheme| as their common ancestor. The second category consists
of  \emph{scheme processors}, which are classes that implement the analysis of a
particular aspect of a single or of a family of communication schemes. Scheme
processors derive from the common ancestor class |SchemeProcessor|. 
...
method that is declared in a class but not implemented. For instance, a class
...
and serves only as a template for implementations of scheme processors. The
example on page~\pageref{sec:perfanalysis} shows how a performance processor,
which is a particular type of scheme processor, is used to plot the achieved
SDR.

Scheme classes and scheme processors interact in the following way. When you
first create an instance of a scheme processor, it generates a random source
sequence of a default variance.\footnote{By default, the source samples are
normally distributed but this behavior may easily be changed by a derived
class.} Next you call the |process()| method of the scheme processor, passing a
list of schemes (\ie, names of scheme classes) and a list of parameters for each
scheme (cf.~the example on page~\pageref{sec:perfanalysis}). (A scheme can have
any number of parameters, like for instance the number of channel input symbols
produced per source symbol.) Internally, the scheme processor then creates an
instance of each specified scheme, passing the source sample sequence to the
constructor of each scheme. Because all schemes operate on the same source
sequence, a fair comparison is guaranteed. 

A scheme processor has a list of SNR values for which each scheme is to be
evaluated. By default, it runs from 0dB to 40dB with increments of~1dB. For
...
After calling a schemes |set_snr()| method, a scheme processor can thus access
...
|set_snr()| and |compute_mse()| and can thus store the MSE for each SNR. This is
precisely what the class |PerformanceProcessor| does. 

Other scheme processors collect different data. For instance, the
|Hybrid2DProcessor| in \lstvref{hybridprocessor} separately computes the errors
due to discrete transmissions and due to continuous transmission for the
...
The common ancestor of all scheme classes is called |Scheme| and is given in
...
class whose instances need to keep a persistent state (through the class
\emph{properties}\footnote{In other object oriented languages, \emph{properties}
...
two categories: theoretical schemes and practical schemes. Schemes of both
categories are (indirectly) derived from |Scheme|, so |Scheme| defines only the
properties and methods common to both categories.
...
We have already mentioned the method |set_snr()| (\lineref{13}). It is called
from outside the class whenever the communication scheme should be simulated for
a new SNR. It first saves the new SNR in its |snr| property (\lineref{14}) and
then calls the abstract method |snr_updated()|. 

The method |compute_mse()| is what allows scheme processors to access a scheme's
...
implement the abstract method themselves, since they inherit the implementation
from~\texttt{X}.}
...
actual communication strategies, but rather compute theoretical values for a
given SNR. The purpose of theoretical scheme is to compare the behavior of a
...
|snr_updated()| just calls |update_mse()|. However, the latter method is itself
an abstract method of |TheoreticalScheme|. This is simply the statement that
``the only thing a theoretical scheme needs to do is to compute the MSE as a
function of the SNR'', expressed programatically.
Listing~\ref{lst:shannonscheme} provides an example implementation of
|compute_mse()|. 


\subsubsection{Practical Schemes}

Practical schemes are classes that actually \emph{implement} a practical
communication scheme (as opposed to theoretical schemes). When the SNR is
updated, a practical scheme must simulate encoding, transmission, and decoding
of the source symbols. Since this procedure (except the particularities of
encoding and decoding) is the same for all practical schemes, it is implemented
in the class |PracticalScheme|. The actual encoding and decoding functions are
declared abstract in |PracticalScheme|, so that classes that implement practical
communication strategies only need to implement these two methods. 

A simplified version of |PracticalScheme| is shown in \lstvref{practicalscheme}.
Its |snr_updated()| method does one thing: call the |run_simulation()| method.
The latter (\lineref{7}) performs exactly the steps mentioned above: the source
is encoded into a channel input sequence (\lineref{9}), transmission is
simulated by adding Gaussian noise to~|x| (\lineref{12}), and the channel
output~|y| is decoded to yield the estimate sequence~|sh| (\lineref{15}).
Finally, the MSE is computed and stored in the |mse|~property (\lineref{18}).
This is the precisely the data that a performance processor accesses when it
calls the scheme's |compute_mse()| method (which, as we have seen, is defined in
the base class |Scheme|). 
...
|PracticalScheme| declares them as abstract. They must be implemented by derived
classes such as |UncodedScheme| in \lstvref{uncoded}.
...
\begin{listing}
  \CodeInput{figures/matlab/SchemeProcessor.m}
  \caption{Simplified implementation of the \texttt{SchemeProcessor} class.}
  \label{lst:schemeprocessor}
\end{listing}

All scheme processors have at the three properties defined in lines~3--5. |s|
stores the source sequence, which is created in the constructor upon
...
these three properties should be self-evident from the code. 

A scheme processor is launched by calling its |process()| method. This method
has two parameters, |schemes| and |parameters|. Both are cell arrays that
...
method. This abstract method allows scheme processor implementations to do
things before the actual processing starts. The most common use of this function
is to allocate a data structure that will hold the data gathered from the
schemes (for an example, see the implementation of |PerformanceProcessor| in
\lstvref{perfproc}).  The |do_processing()| method then performs the actual
processing of the schemes, we will look at it in detail shortly. Finally, the
abstract method |post_process()| is called. In this method, derived classes can
\eg\ display the data gathered from the schemes, save it in a file, etc.

The function |do_processing()|, which starts in \lineref{29}, is at the core of
...
it calls the abstract method |save_scheme_data()|. In their implementation of
this important method, derived classes determine which data to save about the
scheme. For example, the performance processor on
page~\pageref{sec:perfanalysis} uses it to store the MSE. 
...
once, and scheme processors use a slightly involved method to store schemes and
their parameters internally. For these details, the reader so inclined is
invited to peruse the source code.
