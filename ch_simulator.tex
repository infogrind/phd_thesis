...
\emph{Introductory paragraphs go here\ldots}


Before introducing \jscsim, though, this chapter starts with a short section
explaining why object oriented programming is fundamentally suitable to write
simulations.
...
values, and gathers data from each simulation run. 
...
\begin{itemize}
  ...
  \item |post_process()| to post-process the gathered data, which usually means
    plotting it.
\end{itemize}
This is illustrated by the simplified implementation of |PerformanceProcessor|
in \lstref{perfproc}. This class has a single properthy, |mse| (\lineref{4}),
...
For another example of how to use a performance processor, consider the hybrid
...
part~$Q$ and a continuous part~$E$, which are then transmitted across two
channel uses.  The decoder computes the source estimate~$\Sh$ from the separate
estimates~$\Qh$ and~$\Eh$. (This is similar to the communication schemes
introduced in \chapref{mindelbwex}.)
  ...
  classdef Hybrid2DScheme < PracticalScheme
      ...
      methods (Access = 'protected')
          function x = encode(obj, s)
              ...
              obj.e = continuous_part(s);
              
              x(1, :) = scale_to_power_constraint(obj.q);
              x(2, :) = scale_to_power_constraint(obj.e);
          end
          
          function sh = decode(obj, y)
              obj.qh = estimate_qh(y);
              obj.eh = estimate_eh(y);
              sh = combine_estimates(obj.qh, obj.yh);
          end
...
A question of interest is the behavior of the average error $(Q - \Qh)^2$ as a
function of the SNR. The scheme processor |Hybrid2DProcessor| in
  ...
  classdef Hybrid2DProcessor < SchemeProcessor
      ...
      methods (Access = 'protected')
          ...
          function save_scheme_data(obj, scheme, j, k)
              obj.qe(k, j) = mean((scheme.compute_q() - ...
                scheme.compute_qh()).^2);
          end
...
\begin{method}{\meta{class name}(\oarg{sv}, \oarg{s}
  ...
  the class constructor. There are two more arguments of the base class
  constructor, which are the number $k$~of source symbols encoded at a time, and
  the number $n$~of channel symbols produced for every $k$~source symbols.
...
\begin{method}{update_variable_parameters(\obj) (protected)}
  ...
  update any of their own properties that depend on the SNR, if they are not
  explicitly computed as a function of |obj.snr| in |encode()| or |decode()|. It
...
a |compute_| method to access it. We have already seen the example of the
|compute_mse()| function, which is defined in |Scheme|. |PracticalScheme| in
addition implements the following methods.
...
Any derived class should thus implement a |compute_| function for each
...
|SchemeProcessor|. Each such class must implement the three following
abstract methods.
...
In addition, a custom scheme processor can override |create_source_samples()| to
...
A new output module is created by creating a class derived from |OutputModule|
(or from one of its derived classes). Any class derived from |OutputModule| must
...
\subsubsection{Interaction Between Schemes and Scheme Processors}
