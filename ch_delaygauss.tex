...
\chapref{fundamentals}, there exist such joint source-channel codes that perform
as good as any separation-based code, yet only cause minimal delay.

% [Marius] Commented this out, because it is not relevant for the rest of the
% thesis.
%\begin{remark}
...
con\-tin\-u\-ous-valued source when $k$~source symbols are mapped to $n$~channel
...
explained in \secref{measurematching}, \emph{any} input distribution achieves
...
$\Sh' = \E[S] = 0$, with a corresponding $\sdr'$ of~$\ssq$). In \exref{gaussfb},
...
$\snr^{-(n-1)}$ (cf.~\eqvref{gausssepsdr}). If $E$ is transmitted
...
illustrated in \figref{hybridlocus}.
